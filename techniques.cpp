		...
		for (int j = 0; j < n; j += CHUNK_SIZE)
		{
			for (int i_chunk = 0; i_chunk < CHUNK_SIZE; i_chunk++)
			{
				for (int j_chunk = 0; j_chunk < CHUNK_SIZE; j_chunk++)
				{
					...
					results[i + i_chunk] = ((i + i_chunk != j + j_chunk) * 
						(((d < results[i + i_chunk]) * d) + 
		...
		for (int j = 0; j < n; j += CHUNK_SIZE)
		{
			for (int i_chunk = 0; i_chunk < (CHUNK_SIZE / 8); i_chunk++)
			{
				for (int j_chunk = 0; j_chunk < (CHUNK_SIZE / 8); j_chunk++)
				{
					...
					{
						float tmp_x = x_256_j[0];
						float tmp_y = y_256_j[0];
...
/******* [MT | Locality | Branchless | SIMD | AVX2] *******/
void FindClosestDistance_MT_Block_Branchless_SIMD_AVX2(int n, float *x, float *y, float *results, int rankID)
{
	int begin = rankID * (n / NB_THREADS);
	int end = (rankID + 1) * (n / NB_THREADS);
...
double print_elapsed_time(struct timespec start, struct timespec finish)
{
	double elapsed= 0.;
...
int main(int argc, char** argv)
{
	...
	elapsed = print_elapsed_time(start, finish);
	std::cout << "Original:	Time: " << elapsed << std::endl;
